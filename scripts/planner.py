...
# ============================================================
# A* Z OMIJANIEM PRZESZKÓD (roślin)
# ============================================================

class AStarPlanner:
    """
    A* na siatce 2-D z obsługą statycznych przeszkód.

    Każda roślina zajmuje okrąg komórek o promieniu plant_radius [m].
    Przy planowaniu trasy DO konkretnej rośliny przekazujesz jej
    field_id jako exclude_plant — wtedy ta roślina NIE jest przeszkodą
    i łazik może do niej dojechać.
    """

    def __init__(self, grid_size=200, offset=100, cell_size=1.0, plant_radius=3.0):
        self.grid_size    = grid_size
        self.offset       = offset
        self.cell_size    = cell_size
        self.plant_radius = plant_radius

        # Słownik: field_id → zbiór zablokowanych komórek tej rośliny
        # Dzięki temu możemy łatwo wykluczyć jedną roślinę przy planowaniu.
        self._plant_cells: dict = {}   # field_id → set of (gx, gy)

    # ------------------------------------------------------------------
    # Budowanie mapy przeszkód
    # ------------------------------------------------------------------

    def add_plant(self, field_id: str, world_x: float, world_y: float, radius: float = None):
        """Zablokuj komórki siatki wokół rośliny. Zapisuje je pod jej field_id."""
        r = radius if radius is not None else self.plant_radius
        cx, cy = self.world_to_grid((world_x, world_y))
        r_cells = int(math.ceil(r / self.cell_size))

        cells = set()
        for dx in range(-r_cells, r_cells + 1):
            for dy in range(-r_cells, r_cells + 1):
                if math.hypot(dx * self.cell_size, dy * self.cell_size) <= r:
                    cell = (cx + dx, cy + dy)
                    if self.in_bounds(cell):
                        cells.add(cell)

        self._plant_cells[field_id] = cells

    def add_plants(self, plants, radius: float = None):
        """Dodaj wszystkie rośliny z listy obiektów Plant."""
        for plant in plants:
            self.add_plant(plant.name, plant.pos[0], plant.pos[1], radius)

    def clear_obstacles(self):
        self._plant_cells.clear()

    def _blocked_cells(self, exclude_plant: str = None) -> set:
        """Zwraca zbiór wszystkich zablokowanych komórek, opcjonalnie bez jednej rośliny."""
        result = set()
        for field_id, cells in self._plant_cells.items():
            if field_id == exclude_plant:
                continue   # ta roślina jest celem — łazik może do niej dojechać
            result |= cells
        return result
    ...
    def world_to_grid(self, pos):
        x, y = pos
        return int(round(x / self.cell_size)) + self.offset, int(round(y / self.cell_size)) + self.offset

    def grid_to_world(self, cell):
        x, y = cell
        return float(x - self.offset) * self.cell_size, float(y - self.offset) * self.cell_size
    ...
    def heuristic(self, a, b):
        # Euklidesowa — lepsza dla ruchu 8-kierunkowego
        return math.hypot(a[0] - b[0], a[1] - b[1])

    def neighbors(self, cell, blocked: set):
        x, y = cell
        candidates = [
            (x + 1, y),     (x - 1, y),
            (x,     y + 1), (x,     y - 1),
    ...
    def plan(self, start_world, goal_world, exclude_plant: str = None):
        """
        Parametry
        ---------
        start_world    : (x, y) pozycja startowa w układzie świata
        goal_world     : (x, y) cel w układzie świata
        exclude_plant  : field_id rośliny, którą IGNORUJEMY jako przeszkodę
                         (bo łazik jedzie właśnie DO niej)

        Zwraca listę punktów [(x, y), ...] lub [] gdy brak trasy.
        """
        blocked = self._blocked_cells(exclude_plant)

        start = self.world_to_grid(start_world)
        goal  = self.world_to_grid(goal_world)

        # Cel może trafić w zablokowaną komórkę innej rośliny — przesuń go
        if goal in blocked:
            goal = self._nearest_free(goal, blocked)
            if goal is None:
                print("[AStarPlanner] Cel całkowicie zablokowany, fallback: linia prosta.")
                return [goal_world]
        ...
        while open_set:
            ...
            for nb in self.neighbors(current, blocked):
                move_cost = (
                    math.sqrt(2)
                    if abs(nb[0] - current[0]) + abs(nb[1] - current[1]) == 2
                    else 1.0
                )
                new_g = g_score[current] + move_cost
                if nb not in g_score or new_g < g_score[nb]:
                    g_score[nb] = new_g
                    heapq.heappush(open_set, (new_g + self.heuristic(nb, goal), nb))
                    came_from[nb] = current

        print("[AStarPlanner] Brak trasy! Fallback: linia prosta.")
        return [goal_world]
    ...
    def _nearest_free(self, cell, blocked: set, search_radius: int = 8):
        cx, cy = cell
...
class Planista:
    def __init__(
        ...
        self.astar = AStarPlanner(
            ...
            offset=100,
            cell_size=1.0,
            plant_radius=3.0,   # [m] promień blokady wokół rośliny; dobierz do rozmiaru modelu
        )
        self.astar.add_plants(plants)   # zablokuj wszystkie rośliny na mapie A*
    ...
    def execute_field_task(self, rover_info, field, generation):
        ...
        try:
            ...
            path = self.astar.plan(
                self.get_rover_position(rover_info),
                (approach_x, approach_y),
                exclude_plant=field.field_id,  # ta roślina NIE jest przeszkodą — łazik do niej jedzie
            )

            # 2. Jazda waypointami (co 3 — A* 8-kier. generuje więcej punktów niż 4-kier.)
    ...
    def reset_rovers_to_start(self):
        ...
        with self.lock:
            ...
            # Przebuduj mapę przeszkód A* (na wypadek gdyby rośliny zmieniły pozycję)
            self.astar.clear_obstacles()
            self.astar.add_plants([fi.plant for fi in self.fields.values()])
