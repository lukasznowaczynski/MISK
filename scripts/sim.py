...
if os.path.exists(plant_path):
    ...
    for i, (handle, pos, name) in enumerate(zip(plant_handles, plant_positions, plant_names)):
        ...
        try:
            ...
            try:
                marker_img = cv2.aruco.generateImageMarker(aruco_dict, visual_id, tex_res, 1)
            except AttributeError:
                marker_img = cv2.aruco.drawMarker(aruco_dict, visual_id, tex_res)
            ...
            sim.setShapeTexture(plane_handle, new_texture_id, 0, 0, [1, 1])
            sim.removeObject(temp_shape)
            sim.writeTexture(new_texture_id, 0, marker_bytes, 0, 0, tex_res, tex_res)
            sim.setObjectSizeValues(plane_handle, [0.15, 0.15, 0.001])
            
            print(f"stworzono {name} z markerem 5x5 o id aruco: {visual_id})")
            
        except Exception as e:
            print(f"blad {name}: {e}")

else:
    print(f"BŁĄD: Nie można załadować modeli roślin! Plik nie istnieje pod ścieżką: {plant_path}")
